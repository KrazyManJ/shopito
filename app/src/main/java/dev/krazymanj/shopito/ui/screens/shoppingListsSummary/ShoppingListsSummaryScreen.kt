...
import dev.krazymanj.shopito.ui.utils.rememberRelativeTime
...
fun ShoppingListsSummaryScreenContent(
    ...
    LazyColumn(
        ...
        state.shoppingItemsWithDate.onEachIndexed { index, (buyTime,shoppingItems) ->
            val shopCount = shoppingItems
                .map { it.item.location }
                .filter { it != null }
                .distinct().size
            ...
            item(
                ...
                BorderedTitle(
                    title = {
                        Text(
                            text = rememberRelativeTime(buyTime),
                            color = when {
                                DateUtils.isPast(buyTime) -> Primary
                    ...
                    supportingText = {
                        Text(text = pluralStringResource(R.plurals.in_shops_label_count, shopCount, shopCount))
                    }
                )
            }
            items(
        ...
        if (state.shoppingItemsWithoutDate.isNotEmpty()) {
            val shopCount = state.shoppingItemsWithoutDate
                .map { it.item.location }
                .filter { it != null }
                .distinct().size
            ...
            item(
                ...
                BorderedTitle(
                    title = {
                        Text(text = stringResource(R.string.no_buy_time))
                    },
                    modifier = Modifier.padding(top = spacing32),
                    supportingText = {
                        Text(text = pluralStringResource(R.plurals.in_shops_label_count, shopCount, shopCount))
